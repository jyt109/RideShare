...
ride_shares = df[(df['Miles Saved'] > 0)]
ride_shares['Money Saved'] = ride_shares['Miles Saved'].apply(lambda x: '$%.2f' % ((0.4 * 5) * x + 2.5).round(2))
...
ride_shares['point_1'] = '%s,%s,%s' % (row['cpcity'].strip('New York City-'), row['cpname'], row['cploc'])
ride_shares['point_2'] = '%s,%s,%s' % (row['cdcity'].strip('New York City-'), row['cdname'], row['cdloc'])
point_3 = []
point_4 = []
for ind in ride_shares.index:
    row = ride_shares.ix[ind]
    if row['rs_type'] == '1':
        point_3.append('%s,%s,%s' % (row['cdcity'].strip('New York City-'), row['cdname'], row['cdloc']))
        point_4.append('%s,%s,%s' % (row['mdcity'].strip('New York City-'), row['mdname'], row['mdloc']))
    elif row['rs_type'] == '2':
        point_3.append('%s,%s,%s' % (row['mdcity'].strip('New York City-'), row['mdname'], row['mdloc']))
        point_4.append('%s,%s,%s' % (row['cdcity'].strip('New York City-'), row['cdname'], row['cdloc']))
    else:
        print 'problem'
        break
...
#ride_shares.point_1.head()
l = ['c_ride','mride','Time', 'Miles Saved', 'Money Saved', 'Extra Time', 'point_1', 'point_2', 'point_3', 'point_4']
ride_shares[l].to_csv('results.csv')
