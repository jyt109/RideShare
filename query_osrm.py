...
import psycopg2
from db_interface import DBRideShare
import sys

class QueryOSRM(DBRideShare):
    def __init__(self, osrm_path='default', port=6969, 
                 mong_db='ride_routes',
        ...
        if osrmPath == 'default':
            # Depends on where you installed the OSRM engine for querying routes
            self.osrm_path = '/Users/JeffreyTang/Desktop/capstones/ride_sharing/osrm-backend/build'
        self.port = port
    ...
    def osrm_link_input(self):
        return self.sql4osrm(execute=True) #changed from super
